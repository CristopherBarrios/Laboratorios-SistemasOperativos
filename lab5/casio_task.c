...
// esto es deficinicion de masimos y minimos de ofset, todo en segundos
double min_offset,max_offset; //segundos
double min_exec_time,max_exec_time; //segundos
double min_inter_arrival_time,max_inter_arrival_time; //segundos

unsigned int casio_id,jid=1;
struct itimerval inter_arrival_time;

// Esto tiene un delay de 1 ms
void burn_1millisecs() {
	unsigned long long i;
	for(i=0; i<LOOP_ITERATIONS_PER_MILLISEC; i++);
}

// Esto tiene un delay variable
...
// limitacion de param, volviendo a poner valores de inicio
...
// Se restablecen las variables de tiempo
...
// se tienen valores diferentes en el tiempo
...
// se muestran las señales de tiempo
...
// se inicia un task con tiempos y valores
...
// este hace un trabajo, tiempo, inicio y final, y muestra
void do_work(int s)
{
